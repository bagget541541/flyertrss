...
set "RC=%ERRORLEVEL%"
if %RC% NEQ 0 (
    echo [WARN] Step 2 failed, RC=%RC% (continue without details)
    ...
    if defined DETAIL_FILE (
        ...
        echo [WARN] No details json captured, continue without details
    )
)
...
set "DETAIL_ARG="
if defined DETAIL_FILE set "DETAIL_ARG=--detail %DETAIL_FILE%"
"%PYTHON%" llm_daily_gen.py "%LINKS_FILE%" %DETAIL_ARG% --print-path > "%TEMP%\flyert_md_path.txt" 2>&1
set "RC=%ERRORLEVEL%"
if %RC% NEQ 0 (
...
"%PYTHON%" docx_to_wechat.py "%MD_FILE%" --out-dir _site 2>&1
set "RC=%ERRORLEVEL%"
