...
import settings
from publishing_helpers import normalize_posts
...
def _render_cover_pil(info_post, ds, total, bank_count, hot_bank, top_replies,
    ...
    _draw_gradient(draw, width, height, "#e0f2fe", "#f0f9ff")
    ...
    draw.text((width - 40 - meta_bbox[2], 34), meta_text, fill="#94a3b8", font=_FONT_REG)

    tag_badge_colors = {"限时": "#dc2626", "避坑": "#ea580c", "攻略": "#16a34a", "公告": "#2563eb", "实测": "#7c3aed", "讨论": "#78716c"}
    ...
    draw.text((badge_x + tag_bbox[2] + 32, badge_y + 4), f"★ 最热帖 · {replies} 回复", fill="#94a3b8", font=_FONT_REG)
    ...
    if qr_path.exists():
        try:
            ...
            img.paste(qr, (width - 120, height - 140), qr)
        except Exception:
            pass

    draw.line([(0, height - 50), (width, height - 50)], fill="#d1d5db", width=1)
    draw.text((40, height - 40), f"关注 {branding} 每日获取信用卡情报", fill="#64748b", font=_FONT_REG)
