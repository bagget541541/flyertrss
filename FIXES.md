...
### 问题 3：HTML 出现「无有效信息」空卡片

#### 根因
LLM 偶尔生成没有原帖 URL 和 tid 的 `###` 占位卡。原有过滤仅剔除带有
非当日 tid 的卡片，无法识别这类无 tid 卡片，Markdown 解析后被渲染为
「无有效信息」「暂无内容」及问号统计。

#### 修复
`clean_final_markdown()` 在输出前逐卡校验：详情卡必须包含当日 `tid`，
否则整块丢弃。

#### 验证（2026-09-18）
- ✅ 移除 3 条无 URL/tid 的民生银行占位卡
- ✅ 公众号粘贴版保留 14 条有效帖子，无空卡片或问号统计
